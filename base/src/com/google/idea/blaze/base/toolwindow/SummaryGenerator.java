...
public class SummaryGenerator {
  ...
  public static PrintOutput getTaskFinishedSummary(Task task) {
    ...
    stringToPrint
        ...
        .append("\t")
        .append(syncTask.getName())
        .append(" finished");
    if (syncTask.getHasErrors()) {
      stringToPrint.append(" with errors");
    }
    if (syncTask.getSubType() == SubType.BUILD_SHARD) {
      stringToPrint
  ...
  public static void extractInvocationID(Task task, PrintOutput output) {
    if (!(task instanceof SyncTask)) {
      return;
    }
    SyncTask syncTask = (SyncTask) task;
